...
//      else -> return Byte(s.toInt()) // immediate value
  ...
  fun ld(ty: instT, opr1: String, opr2: String) {
    ...
    when (ty) {
      ...
      instT.OP_RDis -> r1.copy(mem[ix.byte2num() + n2]) // LD ACC, [IX+3]
  ...
  fun st(ty: instT, opr1: String, opr2: String) {
    ...
    when (ty) {
      ...
      instT.OP_RDis -> mem[ix.byte2num() + n2].copy(r1) // ST ACC, [IX+3]
      else -> throw SymError("(in  ld)" + ty + ": illegal addressing")
    }
    pc.inc()
  }
  ...
  fun add(ty: instT, opr1: String, opr2: String) {
    ...
    when (ty) {
      ...
      instT.OP_RDis -> r1.add(mem[ix.byte2num() + n2]) // ADD ACC, [IX+2]
...
//    cf = r1.getCF()
  ...
  fun sub(ty: instT, opr1: String, opr2: String) {
    ...
    when (ty) {
      ...
      instT.OP_RDis -> r1.sub(mem[ix.byte2num() + n2]) // SUB ACC, [IX+2]
...
//    cf = r1.getCF()
  ...
  fun and(ty: instT, opr1: String, opr2: String) {
    ...
    when (ty) {
      ...
      instT.OP_RDis -> r1.and(mem[ix.byte2num() + n2]) // AND ACC, [IX+2]
  ...
  fun or(ty: instT, opr1: String, opr2: String) {
    ...
    when (ty) {
      ...
      instT.OP_RDis -> r1.or(mem[ix.byte2num() + n2]) // OR ACC, [IX+2]
  ...
  fun eor(ty: instT, opr1: String, opr2: String) {
    ...
    when (ty) {
      ...
      instT.OP_RDis -> r1.eor(mem[ix.byte2num() + n2]) // EOR ACC, [IX+2]
  ...
  fun cmp(ty: instT, opr1: String, opr2: String) {  
    ...
    when (ty) {
      ...
      instT.OP_RDis -> t.copy(r1).sub(mem[ix.byte2num() + n2]) // CMP ACC, [IX+2]
...
//  val inst_b = spc + op_pat + spc + ident + spc
  ...
  val regex_command = " *(reg|mem|help) *".toRegex()
  val regex_b = inst_b.toRegex()
/*
  val set_prefix = " *set *(0|[1-9][0-9]*) *: *"
  val regex_set_op = (set_prefix + inst_op).toRegex()
  val regex_set_r = (set_prefix + inst_r).toRegex()
  val regex_set_rr = (set_prefix + inst_rr).toRegex()
  val regex_set_rimm = (set_prefix + inst_rimm).toRegex()
  val regex_set_rdir = (set_prefix + inst_rdir).toRegex()
  val regex_set_rdis0 = (set_prefix + inst_rdis0).toRegex()
  val regex_set_rdis = (set_prefix + inst_rdis).toRegex()

  val regex_equ = "([a-zA-Z][a-zA-Z0-9]*) *: *(equ|EQU) *([0-9][0-9]*H) *".toRegex()
*/
  ...
  if (line0 != null) {
    ...
    if (regex_rr.matches(line)) { // OP_RR
    ...
    } else if (regex_command.matches(line)) { // COM
      val (p1) = regex_command.find(line)!!.destructured
      dispatch(cpu, p1, instT.COM, "", "")
